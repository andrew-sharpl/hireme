...
export default function JobDetailPage() {
        ...
        async function fetchJob() {
            try {
                ...
                if (user?.username === response.data.postedByUsername) {
                    const interestedResponse = await api.post<string[]>(`/jobs/${id}/interested`);
        ...
        try {
            ...
            setJob((prev) =>
                prev ? {
                    ...
                    interestCount: prev.interestCount + (response.data.interested ? 1 : -1),
                } : prev
            );
        } catch {
    ...
    if (!job) return <Typography textAlign="center" mt={6}>Job not found.</Typography>;
    ...
    return (
        <Box sx={{ maxWidth: 800, mx: "auto", mt: 4, px: 2 }}>
            ...
            <Typography variant="h4" mb={1}>{job.title}</Typography>
            <Typography variant="body2" color="text.secondary" mb={3}>
                Posted by {job.postedByUsername} · {new Date(job.postedAt).toLocaleDateString()}
            </Typography>

            <Typography variant="body1" mb={3}>{job.body}</Typography>

            <Box sx={{ display: "flex", gap: 2, mb: 3, flexWrap: "wrap" }}>
                <Chip label={`${job.interestCount} interested`} color={job.isInterestedByCurrentUser ? "primary" : "default"} />
                ...
                {isViewer && (
                    ...
                    >
                        {job.isInterestedByCurrentUser ? "Remove Interest" : "Express Interest"}
                    </Button>
                )}
            ...
            {isOwner && interestedUsers.length > 0 && (
                <>
                    <Divider sx={{ mb: 2 }} />
                    <Typography variant="h6" mb={1}>Interested Users</Typography>
